...
{
    public class IntelEmbedBuilder : IIntelEmbedBuilder
    {
        private readonly IDiscordClient _client;
        private readonly IUserIntelProvider _provider;
        private readonly ILogger _log;

        public IntelEmbedBuilder(IDiscordClient client, IUserIntelProvider provider, ILogger<IntelEmbedBuilder> log)
        {
            this._client = client;
        ...
        public async Task<Embed> BuildUserEmbedAsync(IUser user, IGuild guild, CancellationToken cancellationToken = default)
        {
            if (user == null)
                throw new ArgumentNullException(nameof(user));

            this._log.LogDebug("Building intel on user {Username} ({UserID})", user.GetUsernameWithDiscriminator(), user.Id);
            UserIntelContext intel = await this._provider.GetAsync(user.Id, guild?.Id, cancellationToken).ConfigureAwait(false);

            // global user info
            EmbedBuilder embed = new EmbedBuilder()
                .WithAuthor($"Intel on {intel.User.Username}", this._client.CurrentUser.GetSafeAvatarUrl())
            ...
            if (intel.GuildUser != null)
            {
                ...
                {
                    embed.WithAuthor($"Intel on {intel.GuildUser.Nickname}", this._client.CurrentUser.GetSafeAvatarUrl());
                    embed.AddField("Guild nickname", intel.GuildUser.Nickname, true);
                }

                // get roles, respecting hierarchy
                IOrderedEnumerable<IRole> roles = intel.GuildUser.GetRoles(r => r.Id != guild.EveryoneRole.Id).OrderByDescending(r => r.Position);
                if (roles.Any())
        ...
        public async Task<Embed> BuildGuildEmbedAsync(IGuild guild, CancellationToken cancellationToken = default)
        {
            if (guild == null)
            ...
            IEnumerable<IGuildChannel> channels = await guild.GetChannelsAsync(CacheMode.AllowDownload, cancellationToken.ToRequestOptions()).ConfigureAwait(false);
            ITextChannel defaultChannel = await guild.GetDefaultChannelAsync(CacheMode.AllowDownload, cancellationToken.ToRequestOptions()).ConfigureAwait(false);

            embed.WithAuthor($"Intel on {guild.Name}", this._client.CurrentUser.GetSafeAvatarUrl())
