...
{
    // TODO: this depends on auth context being already built
    ...
    {
        private readonly ISharedAccountProvider _sharedAccounts;
        private readonly IGameServerStore _gameServers;

        public UserFeatureProvider(ISharedAccountProvider sharedAccounts, IGameServerStore gameServers)
        {
        ...
        private async Task<bool> CheckSharedAccountsAsync(IDiscordAuthContext context, CancellationToken cancellationToken)
        {
            ...
            SharedAccountFilter filter = SharedAccountFilter.ForUserAccess(context);
            IEnumerable<ISharedAccount> foundResources = await this._sharedAccounts.FindAsync(filter, cancellationToken).ConfigureAwait(false);
