...
namespace TehGM.EinherjiBot.CommandsProcessing
{
    ...
    public class RestrictCommandAttribute : CommandCheckAttribute
    {
        ...
        public override Task<CommandCheckResult> RunCheckAsync(CommandContext context, IServiceProvider services, CancellationToken cancellationToken = default)
        {
            ...
            if (!this.CheckRestriction(context, options.RestrictionGroups))
                return Task.FromResult(CommandCheckResult.Abort("Command restricted"));
            return Task.FromResult(CommandCheckResult.Success);
        }
