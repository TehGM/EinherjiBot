...
{
    public class CanAccessSharedAccount : IBotAuthorizationPolicy<ISharedAccount>
    {
        ...
        public Task<BotAuthorizationResult> EvaluateAsync(ISharedAccount resource, CancellationToken cancellationToken = default)
        {
            if (this._auth.User.IsAdmin()
                ...
                || resource.AuthorizedRoleIDs?.Intersect(this._auth.User.RecognizedDiscordRoleIDs).Any() == true)
